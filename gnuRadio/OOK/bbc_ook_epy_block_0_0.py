...
class blk(gr.sync_block):
    ...
    def __init__(self, dbug=0, msg_len=2**7,cod_len=2**17): #If there is an error, change default here:
        #msg_len = get_top_variable("MESSAGE_LENGTH", default=2**0)
    ...
    def handle_msg(self,msg):
        ...
        msg = pmt.symbol_to_string(msg) # convert to a string
        if self.dbug == 1:
            print("Received Message: " + msg)
        self.codeword = self.codeword + bytearray(int(msg, 2).to_bytes(1, 'big'))
        if self.dbug == 1:
            print("Codeword so far is " + str(self.codeword))
        if len(self.codeword) == self.cod_len:
            #self.td.tic()
            self.decode_and_output()
            #self.td.toc("Decode")
        # else:
        #     self.decode_and_output() # don't think we'll ever get here
            # import pdb
            # pdb.set_trace()
    ...
    def decode_and_output(self):
        ...
        try:
            ...
            for x in result:
                ...
                self.message_port_pub(pmt.intern(self.portNameOut),PMT_msg) # works
            #output_items[0][:] = xtemp[0]  # works
            if self.dbug == 1:
                print("Messages found")
                #print(output_items[0][:].decode())
            #output_items[0] = input_items[0] # for pass through
            #return len(output_items[0]) # GR probably needs this
        except:
            print("Error in parsing output message, probably a hallucination.")
